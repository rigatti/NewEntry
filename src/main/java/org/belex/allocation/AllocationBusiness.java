...
public class AllocationBusiness implements IAllocationBusiness{
	...
	public Allocation getSuppliersEntries(Allocation allocation, RequestParams requestParams) {
		...
		List<SupplierEntry> ses = supplierEntryDAO.getSuppliersByDate(requestParams.getDate());
		for (SupplierEntry se : ses) {

			Supplier supplier = new Supplier();
			supplier.setSupplierCode(se.getSupplierCode());

			HashSet<String> creationReferences = new HashSet<>();

			List<Planning> plannings = planningDAO.getByDateAndSupplier(requestParams.getDate(), supplier.getSupplierCode());
			...
			if (ordersST.hasMoreElements()) {
				while (ordersST.hasMoreElements()) {
					try {
						...
						List<SupplierOrderDetail> sods = supplierOrderDetailDAO.get(supplier.getSupplierCode(), supplierOrderNumber);
						for (SupplierOrderDetail sod : sods) {
/*
							if (StringUtils.equals(sod.getSupplierCode(), Constants.SUPPLIER_STOCK) ) {

								List<CustomerOrderDetail> cods = customerOrderDetailDAO.getByProductCode(sod.getProductCode(), new StringTokenizer(String.valueOf(supplierOrderNumber)));
...
*/
								List<Conditioning> cs = conditioningDAO.get(sod.getProductCode(), sod.getSupplierCode());
	...
	public Allocation getEntriesToAllocate(Allocation allocation, RequestParams requestParams) {
		...
		allocation.setCustomerOrders(new ArrayList<>());
		allocation.setCustomerOrdersToAllocates(new ArrayList<>());
		allocation.setCustomerOrdersAllocateds(new ArrayList<>());
	 ...
	 */
	private List<ProductTraceable> checkAssignedCutomerOrders() {
		// go through all customerOrders and search for a treatedEDDId > 0
		
		List<ProductTraceable> pts = new ArrayList<>();
	...
	private List<CustomerOrder> getCustomerOrders(String currentDate, String supplierCode, int orderNumber, String orderLetter) {
		
		List<CustomerOrder> cos = new ArrayList<>();

		// filter with creationNumber treated for the current date and supplier in the planning
		List<Planning> plannings = planningDAO.getByDateAndSupplier(currentDate, supplierCode);
		...
		List<CustomerOrderDetail> cods = customerOrderDetailDAO.get(orderNumber, orderLetter);
		
		for (CustomerOrderDetail cod : cods) {

			boolean customerOrderConfirmed = false;

			List<SupplierOrderDetail> sods = supplierOrderDetailDAO.get(supplierCode, orderNumber, cod.getProductCode());
			for (SupplierOrderDetail sod : sods) {
				...
				if (StringUtils.equals(cod.getProductCode(), sod.getProductCode())) {
					...
					*/
					...
					List<Conditioning> cs = conditioningDAO.get(sod.getProductCode(), searchConditioningOnSupplier);
			...
			if (customerOrderConfirmed) {
				...
				if (creationNumbers.contains(cod.getOrderNumber() + "-" + cod.getCreationNumber())) {
					...
					List<Conditioning> productConditionings = conditioningDAO.get(cod.getProductCode(), cod.getSupplierCode(), unit);
	...
	private List<ProductTraceable> getArrivalProducts(String supplierCode, int orderNumber, String orderLetter, String arrivalDate) {
		List<ProductTraceable> products = new ArrayList<>();

		// get all treated entries for each order of the given supplierCode and arrivalDate
		List<TreatedEntry> treatedEntries = treatedEntryDAO.get(supplierCode, arrivalDate);
	...
	private List<ProductTraceable> getArrivalProductsNotRequested(String supplierCode, String arrivalDate) {
		List<ProductTraceable> products = new ArrayList<>();
	...
	private ProductTraceable fillProduct(ProductTraceable product, TreatedEntryDetail ted) {
		...
		List<TreatedEntryDetailDestination> tedds = treatedEntryDetailDestinationDAO.getAll(ted.getTreatedEntryDetailId());
	...
	private void sortCutomerOrdersByGets() {
		...
		for (int i=0; i < cos.size(); i++) {
			...
			if (co.getTotalNumberOfUnit() == co.getNumberGets()) {
				if ( ! isExists(allocation.getCustomerOrdersAllocateds(), co)) {
					allocation.addCustomerOrdersAllocated(co);
				}
			} else {
				if ( ! isExists(allocation.getCustomerOrdersToAllocates(), co)) {
					List<SupplierOrderDetail> sods = supplierOrderDetailDAO.get(Constants.SUPPLIER_STOCK, co.getSupplierOrderNumber(), co.getProductCode());
	...
	private boolean isExists(List<CustomerOrder> cos, CustomerOrder coToCheck) {
	 ...
	 */
	private void sortEntryProducts(List<ProductTraceable> ptsExtractedFromDB) {
				...
				for (ProductTraceable product : entry.getProducts()) {
					...
					for (Destination dest : product.getDestinations()) {
						...
						if (dest.isModified() ) {
							...
							TreatedEntry te = treatedEntryDAO.getUnique(entry.getSupplier().getSupplierCode(), entry.getDate(), product.getProductCode());
							TreatedEntryDetail ted = treatedEntryDetailDAO.get(te.getTreatedEntryId());
							...
							for (TreatedEntryDetailDestination tedd : tedds) {
								...
								if ( dest.getCustomerCode().equals(tedd.getCustomerCode()) ) {
									...
									if ( (	dest.isSubstitutionItem() && 
										...
										treatedEntryDetailDestinationDAO.update(tedd);

										treatedEDDId = tedd.getId();

										if (dest.isStockItem()) {

											StockEntry se = stockEntryDAO.get(tedd.getId());
											se.setNumberOfProducts(tedd.getNumberOfUnit());
											stockEntryDAO.update(se);
											
										} else if ( dest.isSupplierReturnItem() ){

											SupplierReturnsEntry sre = supplierReturnsEntryDAO.get(tedd.getId());
											sre.setNumberOfProducts(tedd.getNumberOfUnit());
											supplierReturnsEntryDAO.update(sre);
							...
							if ( ! updated ) {
								...
								treatedEntryDetailDestinationDAO.insert(newTedd);
								...
								if ( dest.isStockItem() ) {
									...
									supplierReturnsEntryDAO.insert(sre);
							...
							if (dest.isSubstitutionItem() && treatedEDDId > 0) {
								...
								List<CustomerOrderDetail> cods = customerOrderDetailDAO.get(dest.getSubstitutionOfProduct(), String.valueOf(dest.getSubstitutionOfSupplierOrderNumber()), dest.getSubstitutionOfSupplierOrderLetter(), dest.getSubstitutionOfCustomerOrderCode(), dest.getSubstitutionOfCustomerOrderNumber());
								for (CustomerOrderDetail cod : cods) {
									if (cod.getNumberGets() == 0) {
										cod.setTreatedEDDId(treatedEDDId);
										customerOrderDetailDAO.update(cod);
									}
								}
							}
						}
	...
	private void confirmCustomerOrders() {
		...
		for (CustomerOrder co : allocation.getCustomerOrdersToAllocates()) {
			...
			if (co.isDeletedOrder() || co.isRenewedOrder()) {
				List<CustomerOrderDetail> sods = customerOrderDetailDAO.get(co.getProductCode(), String.valueOf(co.getSupplierOrderNumber()), co.getSupplierOrderLetter(), co.getOrderCode(), co.getOrderNumber());
				...
				for (CustomerOrderDetail sod : sods) {
					...
					if (co.isDeletedOrder() && sod.getDeletedOrderFlag() != 1) {
						...
						customerOrderDetailDAO.update(sod);
