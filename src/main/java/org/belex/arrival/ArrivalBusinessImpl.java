...
@Setter
@Service("arrivalBusiness")
@Slf4j
public class ArrivalBusinessImpl implements ArrivalBusiness {
	...
	public String storeEntry(Arrival arrival, String forceStoreEntry) {
		...
		if ( ! arrival.isSavedEntry()) {
			...
			if (newEntry) {
				...
				if (treatedEntryTempDAO.save(treatedEntryTemp)) {
					...
					for (Customer customer : arrival.getEntry().getCustomers()) {
						...
						//if ( customer.getCustomerEntry().getAssignedSupplierCode().equals(arrival.getSupplier().getSupplierCode()) ) {
							...
							if ( ! treatedEntryCustomerTempDAO.save(tect)) {
								result = STORE_ERROR;
							}
						//}
					}
	...
	public Event modifyEntry(Arrival arrival, RequestParams requestParams) {
		try {
			...
			List<TreatedEntryTemp> treatedEntryTemps = treatedEntryTempDAO.get(arrival.getSupplier().getSupplierCode(), arrival.getDate(), entry.getProduct().getProductCode());
	...
	public Event removeEntry(Arrival arrival, RequestParams requestParams) {
		...
		if (treatedEntryTemp != null) {
			...
			treatedEntryCustomerTempDAO.deleteAllByEntryId(treatedEntryTemp.getId());
			treatedEntryTempDAO.delete(treatedEntryTemp);
	...
	public Event saveEntry(Arrival arrival) {
		...
		if (treatedEntryTemps == null || treatedEntryTemps.size() == 0) {
			...
			return new Event("removeEntry", String.valueOf(supplierEntryDAO.save(supplierEntry)));
		}
		...
		for (int index = 0 ; index < treatedEntryTemps.size(); index++) { 
			...
			List<TreatedEntryCustomerTemp> treatedEntryCustomerTemps =
			...
			try {
				currentTransactionSucceeded = treatedEntryDAO.insert(treatedEntry);
			} catch (Exception e) {
			...
			if (currentTransactionSucceeded) {
				...
				try {
					currentTransactionSucceeded = treatedEntryDetailDAO.insert(treatedEntryDetail);
				} catch (Exception e) {
				...
				if (currentTransactionSucceeded) {
					...
					for (Customer customer : customers) {
						...
						if (totalUnitRemaining > 0) {
							...
							try {
								currentTransactionSucceeded = treatedEntryDetailDestinationDAO.insert(tedd);
							} catch (Exception e) {
			...
			try {
				...
				if (isfirst) {
					...
					if ( ! supplierEntryDAO.exists(supplierEntry)) {
						currentTransactionSucceeded = supplierEntryDAO.save(supplierEntry);
					}
					...
					SupplierEntry supplierEntry = supplierEntryDAO.getBySupplierEntryId(supplierEntryId);
					supplierEntryId = supplierEntry.getSupplierEntryId();
				}
				...
				currentTransactionSucceeded = supplierEntryDetailDAO.save(supplierEntryDetail);
			...
			try {
				...
				SupplierEntryQuantity supplierEntryQuantity = supplierEntryQuantityDAO.getUnique(supplierEntryId, productCode);
				...
				if (existingSupplierEntryQuantity) {
					currentTransactionSucceeded = supplierEntryQuantityDAO.update(supplierEntryQuantity);
				} else {
					currentTransactionSucceeded = supplierEntryQuantityDAO.save(supplierEntryQuantity);
				}
					...
					for (int i=0; i < customers.size() && totalUnitRemaining > 0; i++) {
						...
						if ( StringUtils.isNotEmpty(supplierOrderNumbers) && StringUtils.isNotEmpty(supplierOrderLetters)) {
							...
							while (numbers.hasMoreTokens() || letters.hasMoreTokens()) {
								...
								cods.addAll(customerOrderDetailDAO.get(productCode, tempNumber, tempLetter ,customer.getCustomerOrder().getOrderCode(), customer.getCustomerOrder().getOrderNumber()));
							}
							...
							for (CustomerOrderDetail cod : cods) {
								...
								//if (cod.getSupplierCode().equals(supplierCode)) {
									...
									if (totalUnitRequestedForCurrentClient > 0) {
										...
										float totalUnitesRecues = totalUnitRequestedForCurrentClient;
										if (totalUnitRemaining < totalUnitRequestedForCurrentClient) {
											totalUnitesRecues = totalUnitRemaining;
											totalUnitRemaining = 0;
										} else {
											totalUnitRemaining -= totalUnitRequestedForCurrentClient; 
										}
		
										cod.setNumberGets(cod.getNumberGets() + totalUnitesRecues);
		
										currentTransactionSucceeded = customerOrderDetailDAO.update(cod);
									}
								//}
							}
						}
	...
	public Event getPending(Arrival arrival) {
		...
		List<TreatedEntryTemp> tets = treatedEntryTempDAO.getBySupplierCodeAndDate(arrival.getSupplier().getSupplierCode(), arrival.getDate());
		...
		if (tets.size() > 0) {
			...
			for (TreatedEntryTemp tet : tets) {
				...
				List<TreatedEntryCustomerTemp> tects =
	...
	public boolean deletePending(Arrival arrival){
		...
		result = treatedEntryTempDAO.deleteAll(arrival.getSupplier().getSupplierCode(), arrival.getDate(), treatedEntryCustomerTempDAO);
	...
	public Arrival getPlannedSuppliers(Arrival arrival) {
		...
		List<Planning> plannings = planningDAO.getByDate(arrival.getSearchSupplierDate());
	...
	public Supplier fillSupplierOrder(Supplier supplier){
		...
		Hashtable<Integer, List<Integer>> orderReferences = new Hashtable<>();
		...
		for ( Order supplierOrder : supplier.getOrders() ) {
			if (orderReferences.containsKey(supplierOrder.getNumber())) {
				List<Integer> creationNumbers = orderReferences.get(supplierOrder.getNumber());
				creationNumbers.add(supplierOrder.getCreationNumber());
				orderReferences.put(supplierOrder.getNumber(), creationNumbers);
			} else {
				List<Integer> creationNumbers = new ArrayList<>();
				creationNumbers.add(supplierOrder.getCreationNumber());
				orderReferences.put(supplierOrder.getNumber(), creationNumbers);
			}
		}
		...
		List<CustomerOrderDetail> cods = customerOrderDetailDAO.get(orderReferences.keySet());
		...
		for (CustomerOrderDetail cod : cods) {
			...
			if (checkCreationNumber(orderReferences, currentOrderNumber, cod.getCreationNumber())) {
				...
				if (treatedItems.contains(String.valueOf(currentOrderNumber) + productCode)) {
					...
					List<SupplierOrderDetail> sods = supplierOrderDetailDAO.get(supplier.getSupplierCode(), currentOrderNumber, productCode);
					for (SupplierOrderDetail sod : sods) {
	
						List<Conditioning> cs = conditioningDAO.get(productCode, supplier.getSupplierCode());
						...
						for (Conditioning c : cs) {
							...
							if ( treatItem ) {
								...
								List<Conditioning> conditionnings = conditioningDAO.getProductsByCode(productCode, true, supplier.getSupplierCode());
	...
	private boolean checkCreationNumber(Hashtable<Integer, List<Integer>> orderReferences, int key, int value) {
		if (orderReferences.containsKey(key)) {
			List<Integer> list = orderReferences.get(key);
	...
	public void selectBasket(Arrival arrival){
		...
		List<PackageDetail> packageDetails = packageDetailDAO.get(entry.getProduct().getProductCode(), new StringTokenizer(orderNumbers, ","));
		
		// get client info + fill entry specifications
		List<CustomerOrderDetail> cods = customerOrderDetailDAO.getByProductCode(entry.getProduct().getProductCode(), new StringTokenizer(orderNumbers, ","));
