...
public class ProductBusiness implements IProductBusiness {
	...
	public void updateProductUnit(Arrival arrival) {
		...
		List<Conditioning> conditionings = conditioningDAO.get(productCode, supplierCode, unit);
	...
	public String createProduct(String description, String ean, Unit unit, String supplierCode, String alternateProductCode){
		...
		try {
			success = productDAO.insert(dbProduct);
			...
			if (success) {
				...
				success = conditioningDAO.insert(conditioning);
				...
				if (success) {
					...
					success = descriptionDAO.insert(DBDescription);
				}
			}
	...
	public Vector<Product> getProducts(String searchValue, String searchExactMatch, String searchType, String searchOnSupplier) {
		...
		if ( ! searchValue.equals("")) {
			...
			if (searchType.equals(SEARCH_TYPE_CODE)) {
				...
				List<db.product.Conditioning> conditioningsDB = conditioningDAO.getProductsByEan(searchValue, stockFilteredSupplierCode);
			...
			if (searchType.equals(SEARCH_TYPE_LABEL)) {
				List<db.product.Product> productsDB = productDAO.getProductsByDescription(searchValue,  stockFilteredSupplierCode);
	...
	*/
	...
	private void updateExistingProduct(db.product.Product oldDbProduct, db.product.Product newDbProduct) {
		...
		if (oldDbProduct != null && newDbProduct != null) {
			...
			List<Conditioning> oldProductConditionning = conditioningDAO.getProductsByCode(oldProductCode, true, null);
			List<Conditioning> newProductConditionning = conditioningDAO.getProductsByCode(newProductCode, true, null);
			...
			*/
			...
			success = productDAO.delete(oldDbProduct);

			if (success) {
				
				if (sameUnitExist) {
					success = conditioningDAO.deleteProductCode(oldProductCode);
				} else {
					success = conditioningDAO.updateProductCode(oldProductCode, newProductCode);
				}

				if (success) {
					success = descriptionDAO.deleteProductCode(oldProductCode);

					if (success) {
						success = treatedEntryDAO.updateProductCode(oldProductCode, newProductCode);
						
						if (success) {
							success = treatedEntryTempDAO.updateProductCode(oldProductCode, newProductCode);

							if (success) {
								success = supplierReturnsEntryDAO.updateProductCode(oldProductCode, newProductCode);

								if (success) {
									success = stockEntryDAO.updateProductCode(oldProductCode, newProductCode);
								}
							}
						}
					}
	...
	private void updateNewProduct(String oldCode, String newCode) {
		...
		success = productDAO.update(oldDbProduct);

		if (success) {
			success = conditioningDAO.updateProductCode(oldCode, newCode);

			if (success) {
				success = descriptionDAO.updateProductCode(oldCode, newCode);

				if (success) {
					success = treatedEntryDAO.updateProductCode(oldCode, newCode);
					
					if (success) {
						success = treatedEntryTempDAO.updateProductCode(oldCode, newCode);

						if (success) {
							success = supplierReturnsEntryDAO.updateProductCode(oldCode, newCode);

							if (success) {
								success = stockEntryDAO.updateProductCode(oldCode, newCode);
							}
						}
					}
				}
	...
	public void deleteProductCode(String productCode) {
		...
		if ( StringUtils.isNotEmpty(productCode) ) {
			...
			if ( dbProduct != null ) {
				...
				success = productDAO.delete(dbProduct);
	
				if (success) {
					success = conditioningDAO.deleteProductCode(productCode.toUpperCase());
	
					if (success) {
						success = descriptionDAO.deleteProductCode(productCode.toUpperCase());
